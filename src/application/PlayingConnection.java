...
public class PlayingConnection {
  ...
  public void changeInfoWinLose(ClientHandler winner, ClientHandler loser)
...
//            System.out.println("!!!!!!" + s);
      ...
      if (parts[0].equals(winner.idName)) {
        ...
        String new_line =
            parts[0] + " " + parts[1] + " " + win_time + " " + lose_time + " " + draw_time + "\n";
      ...
      for (int n = 0; n < 3; n++) {
        if (board[m][n] != 0) {
          for (int i = 0; i < 8; i++) {
            ...
            int tmp_x = m;
            int tmp_y = n;
            while (tmp_x >= 0 && tmp_x < 3 && tmp_y >= 0 && tmp_y < 3
                && board[tmp_x][tmp_y] == board[m][n]) {
              tmp_x = tmp_x + dx[i];
              tmp_y = tmp_y + dy[i];
              times = times + 1;
            }
